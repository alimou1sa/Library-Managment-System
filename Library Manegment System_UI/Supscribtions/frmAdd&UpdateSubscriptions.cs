...
namespace Library_Manegment_System
{
	public partial class frmAdd_UpdateSubscriptions : Form
	{
		...
		private async void frmAdd_UpdateSubscriptions_Load(object sender, EventArgs e)
		{
		...
		private async void btnSave_Click(object sender, EventArgs e)
		{
			...
			if (_Mode == enMode.AddNew) _PaymentDetails.PaymentDate = DateTime.Now;
			...
			if (await  _PaymentDetails.Save())
			{
		     	  ...
		     	  await  clsMembers.ChangeIsMemberActive(ctrlMemberCardWhithFilter1.MemberID, chbIsSubscribtionActive.Checked );
				...
				lblPaymentStatus.Text = clsPayments.GetPaymentStatusText(clsPayments.enPaymentStatus.Paid);
		...
		private void cbSubStatus_SelectedIndexChanged(object sender, EventArgs e)
		{
            if (string.IsNullOrEmpty(cbSubStatus.Text))
                ...
                chbIsSubscribtionActive.Checked= clsMemberSubscriptions.IsSubscriptionActivByStatus((clsMemberSubscriptions.enSubscriptionStatus)clsMemberSubscriptions.GetSubscriptionStatusAsByte(cbSubStatus.Text.Trim() ));           
        }
